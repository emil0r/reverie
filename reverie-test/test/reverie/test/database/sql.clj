...
(fact
 ...
 (let [db (component/start (get-db))]
   (try
     (seed! db)
     (fact "get-pages"
           (map (juxt :serial :name) (sort-by :id (rev.db/get-pages db)))
           => [[1 "Main"] [1 "Main"] [2 "Baz"] [2 "Baz"] [3 "Bar"] [3 "Bar"]])
     (fact "get-pages published=true"
           (map (juxt :serial :name) (sort-by :id (rev.db/get-pages db true)))
           => [[1 "Main"] [2 "Baz"] [3 "Bar"]])
...
(fact
 ...
 (let [db (component/start (get-db))]
   (try
     (seed! db)
     (let [serial (:serial (rev.db/add-page! db {:parent 1 :title ""
                                                 :name "Test page 1"
                                                 :route "/test-page-1"
                                                 :template :foobaz
                                                 :type :page :app ""}))]
     ...
     (fact "same level (move-page)"
           (fact ":before"
                 ...
                 (map
                  ...
                  (page/children (rev.db/get-page db 1 false) db))
                 => [[1 "Bar" 1] [2 "Baz" 1]])
           (fact ":after"
                 (seed! db)
     ...
     (fact "another level (move-page)"
           ...
           (fact ":over"
                 ...
                 (let [{:keys [id]} (rev.db/add-page!
                   ...
                   (map
                    (juxt :order :name :parent)
                    (page/children (rev.db/get-page db 2 false) db)))
